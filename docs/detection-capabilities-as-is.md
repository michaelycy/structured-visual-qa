...
- 分析基线：`master` / `a0f3740` + 当前工作树中的 Python 工程治理调整
- 分析日期：2026-08-28
- 证据状态：本文主表全部为 **Verified（已验证）**，依据可达实现与现有测试
- 不包含：全页扫描件 OCR、通用像素差分、翻译语义正确性判断和复杂表格结构恢复
...
6. 在匹配结果上依次执行布局、内容、术语规则；配置了 OCR Provider 时，再对
   位置尺寸稳定的大图片候选区执行局部 OCR，统一输出 `Issue`。
...
当前可达实现覆盖 **22 个问题场景、20 个实际 Issue 类型**；“额外页面”和“新增
Region”共用 `added_element`，图像指纹与候选区 OCR 共用
`untranslated_raster`，但判别路径和证据不同，因此分别列出。
...
| 翻译完整性 | 多个图像字形簇原样保留 | `untranslated_raster` | 聚合源目标内容指纹相同、位置尺寸稳定的小图片；数量、聚类面积和图片总面积同时达到 Profile 阈值时报告 | High | 不启用 OCR；单张照片、Logo 和零散装饰图跳过 | `pymupdf_parser.py`；`content.py`；`profiles.py` |
| 翻译完整性 | 大图片内局部残留源语言 | `untranslated_raster` | 对位置尺寸稳定但内容发生变化的大图片候选区执行源/目标 OCR，比较脚本字符数、占比和置信度 | Medium/High | 仅在 server 注入 OCR Provider 时运行；默认不做整页 OCR，失败记录状态并降级 | `raster_ocr.py`；`ocr.py`；`pipeline.py`；`server/adapters/ocr/` |
...
| 文本层状态 | 目标页面文字已转曲/矢量化 | `text_vectorized` | 源页文本字符数至少 30，目标页字符数不超过源页的 10% | Info，不扣分 | 命中后抑制该页文本缺失、数字、漏译和术语检测；布局与图片检测继续 | `pipeline.py`；`profiles.py` |
...
默认 Profile 可独立关闭以下检测组：缺失/新增元素、Region 偏移、字号缩小、越界、
重叠、数字不一致、文本漏译、候选区 OCR、Region 尺寸剧变、文字碎片化、字号放大、
隐形文字、文字栅格化和段落水平对齐变化。术语检测由是否注入术语库决定；OCR
开关只有在 server 注入 Provider 时才会触发模型调用。证据：
...
- 翻译语义是否正确、表达是否自然；
- 图片对象位置尺寸不变但内容被替换、且替换内容不涉及可识别文字；
- 水印、线条、阴影、绘制顺序等纯像素层变化；
- 没有结构文本层的整页扫描 PDF（当前 OCR 仅处理已匹配的大图片候选区）；
- 复杂表格行列结构是否保持一致；
...
现有测试覆盖区域偏移、图片缺失、文本合并容错、文本图片重叠、数字一致性、
文本漏译、图像字形簇、候选区 OCR、术语违规、尺寸剧变、文字碎片化、隐形文字、
字号放大、文字转曲与栅格化。当前完整测试共 83 项，其中 4 项基线失败已登记在
T22；因此新增能力只能视为已有针对性证据，不能把完整回归状态描述为全绿。
