...
3. **阈值集中管理**：所有检测阈值和权重只写入 `core/src/document_qa/profiles.py` 的 `RuleProfile`，禁止散落在 CLI、检测器或报告代码中。
...
6. **验收标准**：编码完成后按契约 §11 自查——锁文件、Ruff、渐进 mypy、`python -m compileall -q core/src server/src tests`、双包构建通过；涉及核心行为时完成分阶段真实样例验证，并逐条说明行为不变式。
...
# core CLI：比较与单阶段验证（每阶段确认后再更换 verify-stage）
uv run document-qa examples/un-china-2024-en.pdf examples/un-china-2024-zh.pdf --output tmp/qa-report.json
uv run document-qa examples/un-china-2024-en.pdf examples/un-china-2024-zh.pdf --verify-stage parse --verify-dir tmp/verify-stages
...
uv run mypy core/src/document_qa/schemas core/src/document_qa/profiles.py server/src/document_qa_server/persistence server/src/document_qa_server/settings.py
uv run python -m compileall -q core/src server/src tests

# 仅在用户明确要求时运行 unittest
